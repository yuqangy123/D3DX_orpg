...
void CBaseParticleSystem::InitRenderStatus()
{
	...
	//表示点精灵的尺寸. 这个值可以任意指定视图空间或屏幕空间的点精灵的尺寸, 取决于D3DRS_POINTSCALEENABLE 状态如何设置
	m_pDevice->SetRenderState(D3DRS_POINTSIZE, FtoW(m_size));
	m_pDevice->SetRenderState(D3DRS_POINTSIZE_MIN, FtoW(0.00f));
	//m_pDevice->SetRenderState(D3DRS_POINTSIZE_MAX, m_size);

	// control the size of the particle relative to distance
	//如何根据距离控制点精灵的尺寸—这个距离是点精灵到照相机的距离
	m_pDevice->SetRenderState(D3DRS_POINTSCALE_A, FtoW(0.00f));
	m_pDevice->SetRenderState(D3DRS_POINTSCALE_B, FtoW(0.00f));
	m_pDevice->SetRenderState(D3DRS_POINTSCALE_C, FtoW(1.00f));
...
void CBaseParticleSystem::Render()
{
	if(!m_particles.empty())
	{
		...
		m_pDevice->SetFVF(Particle::_PFVF);
...
CSnow::CSnow(IDirect3DDevice9 *p):
...
void CSnow::resetP(AttributeP &resetAttr)
{
	resetAttr._position = RandomVectorFormBoundBox(m_BoundBox);
	resetAttr._velocity = D3DXVECTOR3(0,2.0f,0);
	ZeroVector3(resetAttr._acceleration);
	resetAttr._vDir=D3DXVECTOR3(0,-1.0f,0);
	resetAttr._lifeTime=.0f;
	resetAttr._age=.0f;
	resetAttr._colorAlive=COLOR_WRITE;
	resetAttr._isAlive=false;
	resetAttr._position.y += abs(m_BoundBox._max.y-m_BoundBox._min.y);
}
...
void CSnow::Update(float timeDelta)
{
	for (list_t<AttributeP>::iterator Itr = m_particles.begin(); Itr != m_particles.end(); ++Itr)
	{
		Itr->_position.y+=(Itr->_velocity.y*Itr->_vDir.y)*timeDelta*10;
...
void CSnow::SetDefAttrubute(const AttributeP &DefA)
{
	m_DefAttribute = DefA;
}

HRESULT CSnow::Init(const string_t &TextFile)
{
	HRESULT hr;
	...
	float fWidth = 400.0f;
	m_BoundBox._max.x=-fWidth;m_BoundBox._max.z=fWidth;m_BoundBox._max.y=m_vOrigin.y+400.0f;
	m_BoundBox._min.x=fWidth;m_BoundBox._min.z=-fWidth;m_BoundBox._min.y=m_vOrigin.y;
	...
	return CBaseParticleSystem::Init(TextFile);
}